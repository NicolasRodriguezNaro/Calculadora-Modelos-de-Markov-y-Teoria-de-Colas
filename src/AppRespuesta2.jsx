...
const App = () => {
  ...
  const markovResults = useMemo(() => {
    ...
    // Si usa vector inicial, calcular evolución de estados
    if (useInitialVector) {
      ...
      // Sin vector inicial, calcular potencias de la matriz
  ...
  }, [transitionMatrix, initialVector, numPeriods, matrixInitialized, numStates, useInitialVector]);
            ...
            <>
              <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                <div className="flex items-center gap-4 mb-4">
                  <h2 className="text-xl font-bold">Opciones de Cálculo</h2>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={useInitialVector}
                      onChange={(e) => setUseInitialVector(e.target.checked)}
                      className="w-5 h-5"
                    />
                    <span className="font-medium">Usar vector de estado inicial</span>
                  </label>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        <th className="p-2 border">De / A</th>
                        {Array(numStates).fill(0).map((_, i) => (
              ...
              {markovResults && markovResults.type === 'vector' && (
                <div className="bg-white rounded-2xl shadow-lg p-6">
                  ...
                  <div className="mb-6">
                    <h3 className="font-medium mb-2">Distribución en el período final (Período {numPeriods})</h3>
                    <ResponsiveContainer width="100%" height={300}>
                      <PieChart>
                        <Pie
                        ...
                        >
                          {markovResults.data[numPeriods].map((_, i) => (
                            <Cell key={`cell-${i}`} fill={`hsl(${(i * 360) / numStates}, 70%, 50%)`} />
                          ))}
                        </Pie>
                      ...
                      <table className="w-full text-sm border-collapse">
                        <thead>
                          <tr className="border-b bg-gray-50">
                            <th className="p-2 text-left border">Período</th>
                            {Array(numStates).fill(0).map((_, i) => (
                              <th key={i} className="p-2 text-right border">Estado {i + 1}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
              ...
              {markovResults && markovResults.type === 'matrix' && (
                <div className="bg-white rounded-2xl shadow-lg p-6">
                  <h2 className="text-xl font-bold mb-4">Resultados - Potencias de la Matriz</h2>
                  <p className="text-gray-600 mb-4">
                    Probabilidades de transición de un estado a otro después de n períodos
                  </p>
                        ...
                        <table className="w-full text-sm border-collapse">
                          <thead>
                            <tr className="bg-gray-50">
                              ...
                              {Array(numStates).fill(0).map((_, i) => (
                                <th key={i} className="p-2 text-center border">Estado {i + 1}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
